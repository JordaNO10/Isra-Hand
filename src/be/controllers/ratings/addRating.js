/**
 * פונקציה זו מטפלת בהוספת דירוג לתרומה.
 * שלבים: בדיקת שדות חובה, הוספת הדירוג למסד הנתונים,
 * ושליחת מיילים רלוונטיים לתורם ולמבקש (באופן אסינכרוני).
 */
...
const addRating = async (req, res) => {
  ...
  try {
    ...
    // שליחת מיילים (לא עוצר במקרה של שגיאה)
    try {
      await sendRated(donation_id, user_id);
    } catch (e) {
      console.warn("Rating email not sent:", e.message);
    }
